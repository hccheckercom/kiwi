 ...
 */
add_action( 'wp_head', 'kiwi_production_test_inject_design_tokens', 5 );
function kiwi_production_test_inject_design_tokens() {
	$primary = '#3b82f6';
	$secondary = '#8b5cf6';
 ...
 */
add_action( 'wp_enqueue_scripts', 'kiwi_production_test_enqueue_fonts', 1 );
function kiwi_production_test_enqueue_fonts() {
	$font_family = 'Inter, sans-serif';
	...
	if ( $font_name && $font_name !== 'sans-serif' && $font_name !== 'serif' ) {
		...
		wp_enqueue_style( 'kiwi-production-test-google-fonts', $font_url, [], null );
	}
}
